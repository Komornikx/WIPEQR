<template>
  <div class="tw-p-2 tw-space-y-2">
    ...
    <v-card variant="outlined" color="blue" background="gray" elevation="10">
      <div class="tw-bg-neutral-800 tw-p-3 tw-text-xl">
        Dostępne bilety: {{ availableTickets.length }}
      </div>
    </v-card>
    <v-card>
      <v-tabs v-model="tab" bg-color="indigo-darken-4">
      ...
      <v-card-text>
        <v-window v-model="tab">
          <v-window-item value="viewer" class="tw-space-y-4">
            <ItemsTable
          ...
          <v-window-item value="honorable" class="tw-space-y-4">
            <ItemsTable
              :items="ticketsStore.getRegisteredTicketsByType(tab)"
              :headers="getHeaders(tab)"
              :pristine="pristineByTab(tab)"
          ...
          <v-window-item value="vip" class="tw-space-y-4">
            <ItemsTable
              :items="ticketsStore.getRegisteredTicketsByType(tab)"
              :headers="getHeaders(tab)"
              :pristine="pristineByTab(tab)"
  ...
  <v-dialog v-model="addingHonorable">
    <v-card min-height="400">
      ...
      <v-card-actions class="tw-flex tw-justify-end tw-w-full">
        <v-btn @click="addHonorable" color="green"> Dodaj </v-btn>
        <v-btn @click="handleAddCancel" color="red"> Anuluj </v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
...
export default {
  ...
  methods: {
    ...
    async handleDrop(registered) {
      this.dropping = true;
      if (!confirm("Na pewno?")) return;

      await $fetch(
        `${this.$config.public.apiURL}/api/tickets?type=viewer&registered=${registered}`,
        {
          method: "DELETE",
          headers: {
            authorization: localStorage.getItem("token"),
